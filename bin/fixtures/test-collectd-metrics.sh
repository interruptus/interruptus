...
curl -X POST -H "Accept:application/json" -H "Content-Type:application/json" http://localhost:8080/api/statement -d '{"name": "eventlogdebug", "query":"SELECT * FROM CollectdMetric", "debug": true}' | python -m json.tool
